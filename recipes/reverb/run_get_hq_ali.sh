...
train_set=tr_simu_1ch
test_sets="et_simu_1ch dt_simu_1ch et_real_1ch dt_real_1ch" #"dt_real_8ch dt_simu_8ch et_real_8ch et_simu_8ch dt_cln et_cln"
...
if [ $stage -le 4 ]; then
  ...
  local_pyspeech/simplify_dictionary.sh data/local/dict \
    conf/phone_map_wsj || exit 1;
  utils/prepare_lang.sh --position_dependent_phones false \
    data/local/dict "<NOISE>" data/local/lang_tmp data/lang
...
if [ $stage -le 13 ]; then
  for x in tr_simu_1ch dt_simu_1ch et_simu_1ch ; do 
    steps/align_fmllr.sh --nj 20 --cmd "$train_cmd" \
      data/$x data/lang exp/tri3 exp/tri3_ali_${x} || exit 1;
  done
fi

if [ $stage -le 14 ] ; then 
 for x in tr_simu_1ch dt_simu_1ch et_simu_1ch; do 
    local_pyspeech/convert_ali.sh exp/tri3 \
      ../wsj/exp/tri3b \
      exp/tri3_ali_$x \
      exp/tri3_ali_wsj_$x || exit 1;
  done
fi
