...
if [ $stage -le 3 ]; then
  ...
  local_pyspeech/simplify_dictionary_libri.sh data/local/dict_nosp conf/phone_map_wsj

  utils/prepare_lang.sh --position_dependent_phones false data/local/dict_nosp \
...
if [ $stage -le 8 ]; then
  ...
  for x in dev_clean test_clean dev_other test_other train_clean_100; do
    steps/align_si.sh --boost-silence 1.25 --nj 10 --cmd "$train_cmd" \
                      data/$x data/lang_nosp exp/mono exp/mono_ali_$x
  done
...
if [ $stage -le 14 ] ; then 
 for x in dev_clean test_clean dev_other test_other train_clean_100; do 
    steps/align_fmllr.sh --nj 30 --cmd "$train_cmd" \
      data/$x data/lang_nosp exp/tri4b exp/tri4b_ali_$x
  done
fi

if [ $stage -le 15 ] ; then 
 for x in dev_clean test_clean dev_other test_other train_clean_100; do 
    local_pyspeech/convert_ali.sh exp/tri4b \
      ../wsj/exp/tri3b \
      exp/tri4b_ali_$x exp/tri4b_ali_wsj_$x || exit 1;
  done
fi
...
if [ $stage -le 18 ] ; then 
 for x in dev_clean test_clean dev_other test_other train_clean_360; do 
    steps/align_fmllr.sh --nj 30 --cmd "$train_cmd" \
      data/$x data/lang_nosp exp/tri5b exp/tri5b_ali_$x
  done
fi

if [ $stage -le 19 ] ; then 
 for x in dev_clean test_clean dev_other test_other train_clean_360; do 
    local_pyspeech/convert_ali.sh exp/tri5b \
      ../wsj/exp/tri3b \
      exp/tri5b_ali_$x exp/tri5b_ali_wsj_$x || exit 1;
  done
fi

if [ $stage -le 20 ] ; then 
 for x in train_clean_460; do 
    steps/align_fmllr.sh --nj 30 --cmd "$train_cmd" \
      data/$x data/lang_nosp exp/tri5b exp/tri5b_ali_$x
  done
fi
