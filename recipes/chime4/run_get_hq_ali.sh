...
enhancement="*isolated_1ch_track*"
#single_blstmmask #### or your method
...
if [ $stage -le 4 ]; then 
  for x in dt05_multi_noisy et05_multi_noisy tr05_multi_noisy; do
    steps/align_fmllr.sh --nj 10 --cmd "$train_cmd" \
      data/$x data/lang exp/tri3b_tr05_multi_noisy exp/tri3b_tr05_multi_noisy_ali_${x} || exit 1;
  done 
fi

if [ $stage -le 5 ] ; then 
 for x in dt05_multi_noisy et05_multi_noisy tr05_multi_noisy; do 
    local_pyspeech/convert_ali.sh exp/tri3b_tr05_multi_noisy \
      ../wsj/exp/tri3b \
      exp/tri3b_tr05_multi_noisy_ali_$x \
      exp/tri3b_tr05_multi_noisy_ali_wsj_$x || exit 1;
  done
fi
exit
