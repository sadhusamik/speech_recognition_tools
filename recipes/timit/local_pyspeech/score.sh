...
model=$2/../${iter}.mdl # assume model one level up from decoding dir.
...
local_pyspeech/timit_norm_trans.pl -i $data/stm -m $phonemap -from 48 -to 38 >$dir/scoring/stm_39phn
...
if [ $stage -le 1 ]; then
  ...
  $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/map_ctm.LMWT.log \
     ...
     local_pyspeech/timit_norm_trans.pl -i - -m $phonemap -from 48 -to 38 '>' \
