...
from nnet_models_cnn import nnetVAECNN, nnetVAECNNNopool
...
def vae_loss_nopool(x, ae_out, latent_out, batch_l, size):
    log_lhood = torch.mean(-0.5 * torch.pow((x - ae_out), 2) - 0.5 * np.log(2 * np.pi * 1))

    latent_out = (pad2list(latent_out[0].view(size[0], size[1], -1), batch_l),
                  pad2list(latent_out[1].view(size[0], size[1], -1), batch_l))
...
def get_args():
    ...
    parser.add_argument("--nopool", action="store_true", help="Set to not use pooling layers")
...
def run(config):
    ...
    for epoch_i in range(config.epochs):
        ...
        for batch_x, batch_l, lab in data_loader_train:
            ...
            if config.nopool:
                loss = vae_loss_nopool(batch_x[:, 0, :, :], ae_out[:, 0, :, :], latent_out, batch_l, s)
            else:
                loss = vae_loss(batch_x[:, 0, :, :], ae_out[:, 0, :, :], latent_out)
