...
from nnet_models_cnn import nnetVaeRsModulation
...
def vae_loss(x, ae_out, latent_out, batch_l):
...
def get_args():
    parser = argparse.ArgumentParser(
        description="Train a Rate Scale Modulation VAE")
    ...
    parser.add_argument("--bn_dim", default=100, type=int, help="Bottle neck dim")
    ...
    parser.add_argument("--beta", type=float, default=5, help="KLD weight factor")
...
def run(config):
    ...
    logging.info('Beta: %f' % (config.beta))
    ...
    model = nnetVaeRsModulation(config.feature_dim, config.num_frames, in_channels,
    ...
    for epoch_i in range(config.epochs):
        ...
        for batch_x, batch_l, lab in data_loader_train:
            ...
            loss = vae_loss(batch_x[:, 0, :, :], ae_out[:, 0, :, :], latent_out, batch_l)

            train_vae_rec_losses.append(loss[0].item())
            train_vae_kl_losses.append(config.beta * loss[1].item())

            (-loss[0] - config.beta * loss[1]).backward()
        ...
        with torch.set_grad_enabled(False):
            ...
            for batch_x, batch_l, lab in data_loader_dev:
                ...
                val_vae_kl_losses.append(config.beta * loss[1].item())
