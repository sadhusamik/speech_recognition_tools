...
def get_args():
    parser = argparse.ArgumentParser(description="Train a CURL Model and Classifier in multitask fashion")
...
def run(config):
    ...
    model = nnetCurlClassifier(config.feature_dim * num_frames, config.encoder_num_layers,
                               config.decoder_num_layers, config.classifier_num_layers, config.hidden_dim,
                               config.hidden_dim_classifier, config.bn_dim, config.comp_num, config.num_classes,
                               config.use_gpu)
    if config.use_gpu:
