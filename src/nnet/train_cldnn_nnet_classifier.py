...
from nnet_models_cnn import nnetCLDNN
...
def get_args():
    parser = argparse.ArgumentParser(description="Train CLDNN Acoustic Model")
    ...
    parser.add_argument("--hidden_size", type=int, default=1024, help="Hidden dimension")
    parser.add_argument("--l_num_layers", type=int, default=2, help="Number of LSTM layers")
    parser.add_argument("--d_num_layers", type=int, default=2, help="Number of DNN layers")
    ...
    parser.add_argument("--num_frames", default=512, type=int, help="Number of frames in eac utterance")
...
def run(config):
    ...
    logging.info('Hidden dimension: %d' % (config.hidden_size))
    logging.info('LSTM number of layers: %d' % (config.l_num_layers))
    logging.info('DNN number of layers: %d' % (config.d_num_layers))
    ...
    model = nnetCLDNN(config.feature_dim, config.num_frames, in_channels, out_channels,
                      kernel, config.hidden_size, config.l_num_layers, config.d_num_layers, config.num_classes)
    if config.use_gpu:
    ...
    for epoch_i in range(config.epochs):
        ...
        if (epoch_i + 1) % config.model_save_interval == 0:
            ...
            torch.save({
                ...
                'l_num_layers': config.l_num_layers,
