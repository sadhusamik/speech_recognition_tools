...
def get_args():
    parser = argparse.ArgumentParser(description="Adapt acoustic model based on RNN-AE Delta PM Measure")

    parser.add_argument("model", help="Feedforward pytorch nnet model")
    parser.add_argument("pm_positive", help="RNN AE performance monitoring model")
    parser.add_argument("pm_negative", help="RNN AE performance monitoring model")
    ...
    parser.add_argument("--time_shift", type=int, default=0, help="Time shift of the negative RNN AE PM")
...
def update(config):
    ...
    pm_model_positive = torch.load(config.pm_positive, map_location=lambda storage, loc: storage)
    ae_model_positive = autoencoderRNN(pm_model_positive['feature_dim'], pm_model_positive['feature_dim'],
                                       pm_model_positive['bn_dim'],
                                       pm_model_positive['encoder_num_layers'], pm_model_positive['decoder_num_layers'],
                                       pm_model_positive['hidden_dim'])
    ae_model_positive.load_state_dict(pm_model_positive['model_state_dict'])

    pm_model_negative = torch.load(config.pm_negative, map_location=lambda storage, loc: storage)
    ae_model_negative = autoencoderRNN(pm_model_negative['feature_dim'], pm_model_negative['feature_dim'],
                                       pm_model_negative['bn_dim'],
                                       pm_model_negative['encoder_num_layers'], pm_model_negative['decoder_num_layers'],
                                       pm_model_negative['hidden_dim'])
    ae_model_negative.load_state_dict(pm_model_negative['model_state_dict'])

    if config.use_gpu:
        ae_model_positive.cuda()
        ae_model_negative.cuda()

    for p in ae_model_positive.parameters():  # Do not update performance monitoring block
        p.requires_grad = False

    for p in ae_model_negative.parameters():  # Do not update performance monitoring block
        p.requires_grad = False
    ...
    for epoch in range(config.epochs):
        ...
        if config.use_gpu:
            batch = torch.empty(0, config.max_seq_len, pm_model_positive['feature_dim']).cuda()
        else:
            batch = torch.empty(0, config.max_seq_len, pm_model_positive['feature_dim'])
        ...
        for utt_id, mat in kaldi_io.read_mat_ark(cmd):
            ...
            if utt_count == config.batch_size:
                ...
                outputs_positive = ae_model_positive(batch_x, batch_l)
                outputs_negative = ae_model_negative(batch_x[:, :-config.time_shift, :], batch_l - config.time_shift)

                optimizer.zero_grad()
                loss_plus = stable_mse(outputs_positive, batch_x)
                loss_minus = stable_mse(outputs_negative, batch_x[:, config.time_shift:, :])
