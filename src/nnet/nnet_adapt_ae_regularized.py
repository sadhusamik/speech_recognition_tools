...
def get_args():
    parser = argparse.ArgumentParser(description="Adapt acoustic model based on RNN-AE PM Measure with Regularization")
    ...
    for epoch in range(config.epochs):
        ...
        for utt_id, mat in kaldi_io.read_mat_ark(cmd):
            ...
            if utt_count == config.batch_size:
                ...
                loss_all = loss + config.reg_weight * loss_reg
                loss_all.backward()
                optimizer.step()
        ...
        print_log = "Epoch: {:d} update, Tr MSE Loss: {:.3f} Regularization Loss: {:.3f} :: Dev loss: {:.3f} :: Dev FER: {:.2f}".format(
            epoch,
            np.mean(tr_losses),
            np.mean(reg_losses),
            np.mean(val_losses),
