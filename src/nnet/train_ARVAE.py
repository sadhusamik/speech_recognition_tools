...
from nnet_models import nnetARVAE
...
def vae_loss_ar(x, ae_out, latent_out, ar_steps, batch_l, use_gpu=True):
    log_lhood = Variable(torch.FloatTensor([0]))
    if use_gpu:
        log_lhood = log_lhood.cuda()

    for idx, step in enumerate(ar_steps):
        if step == 0:
            log_lhood += torch.mean(
                -0.5 * torch.pow((pad2list(x, batch_l) - pad2list(ae_out[idx], batch_l)), 2) - 0.5 * np.log(
                    2 * np.pi * 1))
        else:
            log_lhood += torch.mean(
                -0.5 * torch.pow(
                    (pad2list(x[:, :-step, :], batch_l - step) - pad2list(ae_out[idx][:, step:, :], batch_l - step)),
                    2) - 0.5 * np.log(
                    2 * np.pi * 1))
...
def get_args():
    parser = argparse.ArgumentParser(
        description="Train an Autoregressive VAE")
    ...
    parser.add_argument("--ar_steps", default="3,5", help="Autoregressive Steps")
...
def run(config):
    ...
    logging.info('Autoregressive Steps: %s' % (config.ar_steps))

    sys.stdout.flush()

    ar_steps = config.ar_steps.split(',')
    ar_steps = [int(x) for x in ar_steps]
    ar_steps.append(0)

    model = nnetARVAE(config.feature_dim * num_frames, config.encoder_num_layers,
                      config.decoder_num_layers, config.hidden_dim, config.bn_dim, 0, len(ar_steps), config.use_gpu)
    ...
    for epoch_i in range(config.epochs):
        ...
        for batch_x, batch_l, lab in data_loader_train:
            ...
            loss = vae_loss_ar(batch_x, ae_out, latent_out, ar_steps, batch_l, config.use_gpu)
