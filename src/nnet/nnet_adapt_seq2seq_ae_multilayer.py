...
def get_args():
    parser = argparse.ArgumentParser(
        description="Adapt acoustic model based on seq2seq RNN-AE PM Measure on multiple layers")
    ...
    # Other options
    parser.add_argument("--use_softmax", action="store_true", help="Use softmax for the final layer")
...
def update(config):
    ...
    for path in pm_paths:
        ...
        ae_model = seq2seqRNNAE(pm_model['feature_dim'], pm_model['feature_dim'], pm_model['encoder_num_layers'],
                                pm_model['decoder_num_layers'], pm_model['hidden_dim'], config.use_gpu)
        ...
        for utt_id, mat in kaldi_io.read_mat_ark(cmd):
            ...
            if config.use_gpu:
                if config.use_softmax:
                    post = sm(out[1]) - Variable(torch.FloatTensor(means[0])).cuda()
                else:
                    post = out[1] - Variable(torch.FloatTensor(means[0])).cuda()
            else:
                if config.use_softmax:
                    post = sm(out[1]) - Variable(torch.FloatTensor(means[0]))
                else:
            ...
            if utt_count == config.batch_size:
                ...
                for idx in range(num_pm_models):
                    ...
                    loss = stable_abs(outputs, batch_x)
                    loss_all *= loss
                    tl = tr_losses[idx]
                    tl.append(loss.item())
                    tr_losses[idx] = tl
