...
def get_args():
    parser = argparse.ArgumentParser(description="Adapt acoustic model with light self supervision with regularization")
    ...
    parser.add_argument("regularized_egs", help="Regularization egs .pt file to regularize")
    parser.add_argument("cmvn", help="cmvn file for posteriors")

    # Other options
    parser.add_argument("--reg_weight", type=float, default=1, help="Weight to multiply the regularized gradient")
    ...
    parser.add_argument("--batch_size", type=int, default=50000, help="Number of utterances to use for adaptation")
...
def update(config):
    ...
    data_loader_check = torch.utils.data.DataLoader(dataset, batch_size=config.batch_size, shuffle=True)
    ...
    reg_set = list(torch.utils.data.BatchSampler(sampler, batch_size=config.batch_size, drop_last=False))
            ...
            for utt_id, mat in kaldi_io.read_mat_ark(cmd):
                ...
                if utt_count == 64:
                    ...
                    if config.time_shift == 0:
                        ...
                        loss = criterion_ae(outputs, batch_x[:, config.time_shift:, :])
        ...
        if sup_update:
            ...
            for batch_x, batch_l in data_loader:
                ...
                if config.use_gpu:
                    ...
                    reg_fer.append(compute_fer(reg_x.cpu().data.numpy(), reg_l.cpu().data.numpy()))
                else:
                    tr_fer.append(compute_fer(batch_x.data.numpy(), batch_l.data.numpy()))
                    reg_fer.append(compute_fer(reg_x.data.numpy(), reg_l.data.numpy()))
