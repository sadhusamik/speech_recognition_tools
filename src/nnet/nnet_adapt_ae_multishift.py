...
def get_args():
    parser = argparse.ArgumentParser(description="Adapt acoustic model based on RNN-AE PM Measure with multiple shifts")

    parser.add_argument("model", help="Feedforward pytorch nnet model")
    parser.add_argument("pms", help="List of RNN AE performance monitoring model for different shifts")
    ...
    parser.add_argument("cmvn", help="Global cmvn file for nnet output")
    ...
    parser.add_argument("--time_shifts", type=int, default=0, help="Time shifts of the RNN AE PM models")
...
def update(config):
    ...
    pm_shifts = config.time_shifts.split(',')
