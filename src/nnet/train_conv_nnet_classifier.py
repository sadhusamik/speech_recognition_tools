...
from nnet_models_cnn import nnetCNNClassifier
...
def get_args():
    parser = argparse.ArgumentParser(description="Train Convolutional Feedforward Acoustic Model")
...
def run(config):
    ...
    model = nnetCNNClassifier(config.feature_dim, config.num_frames, in_channels, out_channels,
                              kernel, config.num_classes)
    if config.use_gpu:
