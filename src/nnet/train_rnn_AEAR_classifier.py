...
def get_args():
    parser = argparse.ArgumentParser(
        description="Train Feedforward Acoustic Model an Autoencoder and Autoregressive Worker with multitask training")
    ...
    parser.add_argument("--time_shift", default=3, type=int, help="Autoregressive Time Shift")
...
def run(config):
    ...
    model = nnetAEClassifierMultitaskAEAR(config.feature_dim * num_frames, config.num_classes,
                                          config.encoder_num_layers,
                                          config.classifier_num_layers, config.ae_num_layers,
                                          config.hidden_dim, config.bn_dim, config.time_shift)
    if config.use_gpu:
    ...
    for epoch_i in range(config.epochs):
        ...
        for batch_x, batch_l, lab in data_loader_train:
            ...
            loss_ar = criterion_ae(ar_out, pad2list(batch_x[:, config.time_shift:, :], batch_l - config.time_shift))
        ...
        print_log += " || AR Loss (Train) : {:.3f} :: AR Loss (Dev) : {:.3f} ".format(ep_ar_tr[-1], ep_ar_dev[-1])
